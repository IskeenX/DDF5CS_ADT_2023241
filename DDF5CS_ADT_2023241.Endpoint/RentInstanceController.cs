...
namespace DDF5CS_ADT_2023241.Endpoint
{
    ...
    public class RentInstanceController : ControllerBase
    {
        ...
        public RentInstanceController(IRentInstanceLogic rentInstanceLogic) { _rentInstanceLogic = rentInstanceLogic; }
        [HttpGet("{id}")]
        public IActionResult GetRentInstance(int id) {
            var rentInstance = _rentInstanceLogic.GetRentInstance(id);
            if (rentInstance == null) return NotFound();
        ...
        {
            if (rentInstance == null) return BadRequest();
            try {
        ...
        {
            if (rentInstance == null || id != rentInstance.RentInstanceId) return BadRequest();
            try {
